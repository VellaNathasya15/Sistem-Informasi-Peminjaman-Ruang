...
class Dashboard extends CI_Controller 
{
	...
	*/
	...
	public function data_inventory()
	{
		...
		// Memanggil fungsi lihat data pada Model Inventory
		$query   = $this->model_inventory->tampil()->result_array();

		// Data disimpan sementara pada array
		$halaman = array
		(
			'tb'   => $query,
			'hal'  => 'Kelola Data Inventory'
		);

		$this->template->utama('pages/halaman-inventory', $halaman);
	}
	public function buat_inventory()
	{
		// Menerima input, lalu disimpan pada variabel-variabel berikut
		$id_inventory  = $this->input->post('id_inventory');
		$namainventory = $this->input->post('nama_inventory');
		...
		$query    = $this->model_inventory->buat($data);

		if ($query)
		{
			redirect(base_url('dashboard/data-inventory'));
		}
		else
		{
			echo "Proses penambahan data gagal.";
	...
	public function perbarui_inventory()
	{
		// Menerima input, lalu disimpan pada variabel-variabel berikut
		$id_inventory  = $this->input->post('id_inventory');
		$namainventory = $this->input->post('nama_inventory');
		...
		// Data yang akan diperbarui
		$data     = array('nama_inventory' => $namainventory);

		// ID inventory digunakan sebagai pengenal data mana yang akan diperbarui
		$where    = array('id_inventory' => $id_inventory);

		$query    = $this->model_inventory->perbarui($data, $where);
	...
	public function hapus_inventory()
	{
		...
		// ID inventory digunakan sebagai pengenal data mana yang akan dihapus
		$where   = array('id_inventory' => $id_inventory);

		$query   = $this->model_inventory->hapus($where);
	...
	*/
	...
	public function data_peminjaman()
	{
		...
		$query_d = $this->model_inventory->tampil()->result_array();
	...
	public function pinjam_inventory()
	{
		// Menerima input, lalu disimpan pada variabel-variabel berikut
		$id_peminjaman = $this->input->post('id_peminjaman');
		$peminjam      = $this->input->post('peminjam');
		...
		(
			...
			'waktu_pengajuan'  => $w_pengajuan,
			'keterangan'       => $keterangan,
			'inventory_dipinjam' => $nama_inventory,
			'verifikasi_bp'    => '-',
			'verifikasi_kabag' => '-',
			'status_pinjam'    => 'Menunggu'
		);
		...
		// Nama ruangan digunakan sebagai pengenal data mana yang akan diperbarui
		$where    = array('nama_inventory' => $nama_inventory);

		$query_inventory = $this->model_inventory->perbarui($data, $where);

		if ($query && $query_inventory)
		{
			redirect(base_url('dashboard/data-peminjaman'));
		}
		else
	...
	public function selesaikan_peminjaman()
	{
		...
		// Pengubahan status pada tabel inventory dari "Sedang digunakan" menjadi "Tersedia"
		// Data yang akan diperbarui
		$data     = array('status' => 'Tersedia');
